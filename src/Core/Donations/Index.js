...
const Index = () => {
  ...
  const getPosts = () => {
    ...
    axios({
      method: "get",
      url: `${process.env.REACT_APP_BACKEND}/donation/posts`,
    })
      .then((response) => {
  ...
  const deletePost = (postId) => {
    ...
    axios({
      method: "delete",
      url: `${process.env.REACT_APP_BACKEND}/donation/post/${postId}`,
    }).then((response) => {
      ...
      <Row>
        ...
        <Col className="categories-content" lg={10}>
          <Row>
            <Col lg={10}>
              <h4>Our Donations</h4>
              <p>Below are the Donations that currently added.</p>
            </Col>
          </Row>
          <hr />
